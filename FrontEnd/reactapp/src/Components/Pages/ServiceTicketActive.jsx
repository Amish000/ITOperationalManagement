...
import { TicketServicesActive, DeleteTicket, UpdateTicketStatus, GetTicketDetails, PredictCategory } from "../../services/ticketServiceAdmin";
import { EditClientTicket, getClientTicketServiceActive } from "../../services/ticketServiceClient";
...
const ServiceTicketActive = () => {
  ...
  const totalItem = pagination.totalItems;
  ...
  const fetchServices = async () => {
    try {
      ...
      if (userRoles.includes("SuperAdmin")) {
        const response = await TicketServicesActive({
          search,
        ...
        const response = await getClientTicketServiceActive({
          search,
          sortColumn: pagination.sortColumn,
          sortOrder: pagination.sortOrder,
          page: pagination.page + 1,
  ...
  const handleUpdateStatus = async (ticketIdToUpdate, newStatus) => {
    try {
      console.log(ticketIdToUpdate, newStatus);
      await UpdateTicketStatus(ticketIdToUpdate, newStatus);

      const response = await GetTicketDetails(ticketIdToUpdate);
      setTicket(response.data)
          ...
          <Box sx={{ p: 1, width: '100%', height: "600px" }}>
            {loading ? (
                            ...
                            {userRoles.includes("User") && (
                              <>
                                ...
                                <Button
                                  ...
                                  startIcon={<EditIcon />}
                                  onClick={() => openEditBox(service.id)}
                                >
                                  Edit
                                </Button>
                              </>
              ...
              <Typography variant="h4" sx={{ textAlign: 'center', mt: 5 }}>
                {hasInitialData ? 'No matching results found.' : 'No Active Tickets'}
              </Typography>}
          </Box>
        </Box>
      </Box>
...
export default ServiceTicketActive;
